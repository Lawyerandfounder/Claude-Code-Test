...
function addBotMessage(message, showOptions = false) {
    ...
    // Scroll to show the top of this new bot message
...
async function processUserMessage(message) {
    ...
    try {
        ...
        // Include user name and email for tracking in n8n
        const apiUrl = `/api/chat?message=${encodeURIComponent(message)}&name=${encodeURIComponent(userData.name)}&email=${encodeURIComponent(userData.email)}`;
...
function scrollToBottom() {
    // Scroll to top of the chat to show the latest message from the beginning
